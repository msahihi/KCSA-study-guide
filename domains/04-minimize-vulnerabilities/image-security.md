...
1. [Vulnerability Scanning with Trivy](#vulnerability-scanning-with-trivy)
1. [Image Signing and Verification](#image-signing-and-verification)
1. [Container Registry Security](#container-registry-security)
1. [Image Pull Secrets](#image-pull-secrets)
1. [Building Secure Images](#building-secure-images)
1. [Image Policy Enforcement](#image-policy-enforcement)
...
1. **Outdated Dependencies**: Old libraries with known exploits
1. **Embedded Secrets**: Passwords, API keys in image layers
1. **Excessive Permissions**: Running as root, unnecessary capabilities
1. **Bloated Images**: Unnecessary packages increase attack surface
1. **Unverified Sources**: Images from untrusted registries
...
1. **Image Signing**: Use Cosign to sign and verify images
1. **Registry Security**: Use private registries with authentication
1. **Image Pull Secrets**: Securely manage registry credentials
1. **Secure Builds**: Use minimal base images and multi-stage builds
1. **Policy Enforcement**: Block untrusted or vulnerable images at admission
