...
2. **Unnecessary services**: Extra attack vectors
3. **Weak access controls**: Poor SSH/sudo configuration
4. **Insecure defaults**: Default passwords, open ports
5. **Excessive permissions**: SUID binaries, world-writable files
...
2. **Keep Updated**: Enable automatic security updates, monitor CVEs
3. **Defense in Depth**: Multiple layers of security (firewall, SELinux/AppArmor, audit)
4. **Least Privilege**: Restrict access, use sudo instead of root, key-based SSH
5. **Audit Everything**: Enable comprehensive logging and monitoring
6. **Regular Audits**: Use kube-bench, scan for vulnerabilities
7. **Immutable Infrastructure**: Consider immutable OS like Bottlerocket or Talos
8. **Incident Response**: Have a plan for responding to compromised hosts
