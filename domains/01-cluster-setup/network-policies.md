...
1. **Compliance**: Many security frameworks require network segmentation between different tiers of applications.
1. **Blast Radius Reduction**: Isolating workloads limits the impact of security incidents.
1. **Multi-tenancy**: NetworkPolicies help enforce isolation between different teams or applications sharing a cluster.
...
1. **Kubernetes 1.7+**: NetworkPolicy has been stable since v1.7.
...
1. **Policy Types**: There are two types of traffic you can control:
...
1. **Additive Nature**: Multiple NetworkPolicies can select the same pod, and the union of all rules applies.
...
1. **Namespace Selectors**: Allow traffic from/to all pods in specific namespaces
1. **IP Blocks (CIDR)**: Allow traffic from/to specific IP ranges
...
1. **Use Meaningful Labels**: Create a consistent labeling strategy for your pods and namespaces.
...
1. **Monitor and Audit**: Regularly review NetworkPolicies and remove unused ones.

1. **Layer Your Security**: NetworkPolicies are one layer. Use them in conjunction with:
...
1. **Use Network Policy Tools**: Consider tools like:
...
1. By default, Kubernetes allows all pod-to-pod communication.
1. Once a NetworkPolicy selects a pod, traffic is denied unless explicitly allowed.
1. Multiple NetworkPolicies are additive (union of all rules).
1. Always include DNS in egress rules.
1. NetworkPolicies are namespaced resources.
1. Empty `podSelector: {}` selects all pods in the namespace.
1. Use namespace labels for cross-namespace policies.
1. Test policies thoroughly before production deployment.
1. NetworkPolicies only work at Layer 3/4 (IP/port), not Layer 7 (HTTP).
...
1. Practice creating default deny policies
1. Experiment with different selector combinations
1. Learn about [CIS Benchmarks](./cis-benchmarks.md) next
