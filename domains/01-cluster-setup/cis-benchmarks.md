...
2. **Compliance**: Many regulatory frameworks (PCI-DSS, HIPAA, SOC 2) reference CIS Benchmarks.
3. **Comprehensive**: Covers all cluster components (master nodes, worker nodes, policies).
4. **Actionable**: Provides specific recommendations with remediation steps.
5. **Regularly Updated**: Maintained to reflect current Kubernetes versions and threats.
...
2. **Automate Scanning**: Integrate kube-bench into CI/CD pipelines:
```yaml
# In CI/CD pipeline
- name: Security Audit
  run: |
...
3. **Track Remediation**: Use issue tracking to manage failed checks:
```bash
# Export to JSON for processing
./kube-bench --json > results.json
```

4. **Document Exceptions**: Some findings may be acceptable in your environment. Document why:
```yaml
...
5. **Test Changes**: Always test remediations in non-production first:
```bash
# Test in staging
...
6. **Version-Specific Benchmarks**: Use the correct benchmark version for your Kubernetes version:
```bash
...
2. kube-bench automates benchmark checking.
3. Not all recommendations apply to every environment.
4. Prioritize remediation based on risk and impact.
5. Test changes in non-production environments first.
6. Regular audits catch configuration drift.
7. Document exceptions and accepted risks.
8. Cloud providers handle some controls differently.
9. Use version-appropriate benchmarks.
10. Combine with other security practices (PSS, RBAC, Network Policies).
...
2. Run kube-bench on a practice cluster
3. Practice interpreting and remediating findings
4. Learn about [Ingress and Service Security](./ingress-service-security.md) next
