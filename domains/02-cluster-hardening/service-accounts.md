...
2. **ca.crt**: CA certificate to verify API server's TLS certificate
3. **namespace**: The pod's namespace
...
2. **Token location**: `/var/run/secrets/kubernetes.io/serviceaccount/`
3. **Disable automounting**: Use when pod doesn't need API access
4. **RBAC integration**: ServiceAccounts are RBAC subjects
5. **Subject format**: `system:serviceaccount:NAMESPACE:NAME`
6. **Cross-namespace**: ServiceAccounts can access other namespaces with appropriate RoleBinding
7. **Best practice**: One ServiceAccount per application with least privilege
