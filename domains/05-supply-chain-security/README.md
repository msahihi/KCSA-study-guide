...
1. **Hands-on Practice**: Supply chain security requires practical knowledge. Complete all labs multiple times to build muscle memory.

1. **Image Scanning Priority**: Focus on HIGH and CRITICAL vulnerabilities. The exam may ask you to identify and prioritize issues.

1. **Signature Verification**: Understand the difference between signed and unsigned images, and how to enforce signature verification.

1. **Common Scenarios**: Be prepared for tasks like:
...
1. **Time Management**: Scanning large images can take time. Know how to scan specific components or use faster scan modes.

1. **Documentation Access**: Familiarize yourself with Trivy and Cosign documentation on their official sites. These may be accessible during the exam.
...
- [CNCF Supply Chain Security Best Practices](https://www.cncf.io/blog/)
...
1. **Fail Fast**: Block deployment of images with HIGH/CRITICAL vulnerabilities
1. **Automate Scanning**: Integrate scanning into CI/CD pipelines
1. **Regular Updates**: Keep base images and dependencies up to date
1. **Minimal Images**: Use distroless or minimal base images to reduce attack surface

### Image Signing

1. **Always Sign**: Sign all production images
1. **Verify Before Deploy**: Enforce signature verification in admission control
1. **Rotate Keys**: Regularly rotate signing keys
1. **Keyless When Possible**: Use Sigstore for keyless signing
1. **Audit Trails**: Maintain logs of signing operations

### Registry Security

1. **Private Registries**: Use private registries for production images
1. **Strong Authentication**: Implement strong authentication and RBAC
1. **Encrypted Transit**: Use TLS for all registry communications
1. **Access Control**: Limit who can push/pull images
1. **Regular Audits**: Audit registry access logs

### SBOM Management

1. **Generate Always**: Create SBOMs for all images
1. **Version Control**: Store SBOMs with source code
1. **Track Dependencies**: Monitor dependencies for vulnerabilities
1. **License Compliance**: Use SBOMs for license auditing
1. **Automation**: Automate SBOM generation in CI/CD
...
1. **Quick Scanning**: Practice fast scanning techniques
1. **Result Interpretation**: Quickly identify actionable findings
1. **Troubleshooting**: Common issues with signing and verification
1. **Time Boxing**: Allocate 20-25 minutes for supply chain questions (20% of exam)

## Next Steps

1. Start with the [Image Scanning](./image-scanning.md) guide
1. Install required tools (Trivy, Cosign)
1. Complete each lab in sequence
1. Practice with real-world images
1. Review security best practices regularly
