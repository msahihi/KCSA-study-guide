...
2. **Integrity**: Ensure the image hasn't been modified or corrupted
3. **Non-Repudiation**: Publisher cannot deny signing the image
4. **Supply Chain Security**: Prevent supply chain attacks
5. **Compliance**: Meet regulatory requirements for software provenance
6. **Trust**: Build confidence in your deployment pipeline
...
2. **OCI Registry Storage**: Signatures stored alongside images
3. **Keyless Signing**: No key management required (using OIDC)
4. **Hardware Token Support**: YubiKey, PIV cards
5. **Policy Enforcement**: Integration with admission controllers
6. **SBOM Attestation**: Attach and verify SBOMs
7. **Multiple Signature Formats**: Support for various formats
...
2. Receives short-lived certificate from Fulcio (certificate authority)
3. Signs image with ephemeral key
4. Signature and certificate stored in Rekor (transparency log)
5. Short-lived key is discarded
6. Verification checks transparency log
...
2. Sign image with ephemeral key
...
2. Cosign is the standard CNCF tool for signing
3. Keyless signing eliminates key management burden
4. Signatures are stored in OCI registries
5. Always verify signatures before deployment
6. Use immutable digests, not mutable tags
7. Integrate signing into CI/CD pipelines
8. Combine signing with admission controllers
9. Rotate signing keys regularly
10. Maintain comprehensive audit trails

## Exam Tips

1. Know Cosign command syntax thoroughly
2. Understand difference between key-based and keyless signing
3. Practice signing and verifying images quickly
4. Know how to generate and manage key pairs
5. Understand signature storage in OCI registries
6. Be able to troubleshoot verification failures
7. Know how to integrate with admission controllers
...
2. Practice signing various images
3. Learn about [Registry Security](./registry-security.md) next
