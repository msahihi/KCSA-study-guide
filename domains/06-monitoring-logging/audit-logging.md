...
1. Restrict access to logs (RBAC, filesystem permissions)
1. Send logs to centralized, secure storage
1. Enable log integrity checking (hashing)
1. Monitor for log tampering
...
1. Log path not writable
1. Policy file syntax error
...
1. Increase omitStages
1. Use Metadata instead of Request level
...
1. **The four stages**: RequestReceived, ResponseStarted, ResponseComplete, Panic
1. **Rule precedence**: First match wins
1. **Common audit scenarios**: Secret access, pod exec, RBAC changes
1. **API server flags**: --audit-policy-file, --audit-log-path
1. **How to read audit events**: Key fields like user, verb, objectRef
1. **Performance impact**: RequestResponse is most expensive
...
1. Audit policies control what and how much is logged
1. Four levels: None, Metadata, Request, RequestResponse
1. First matching rule wins (order matters)
1. Balance security needs with performance impact
1. Always log security-sensitive operations
1. Protect audit logs - they contain sensitive data
1. Use jq for efficient log analysis
