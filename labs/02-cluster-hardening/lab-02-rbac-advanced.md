...
2. **Reusability**: ClusterRoles are reusable across namespaces with RoleBindings
3. **Aggregation**: Break down complex roles into modular components with labels
4. **Built-in roles**: Use admin, edit, view when they fit your needs
5. **Cross-namespace**: ServiceAccounts can access other namespaces with appropriate RoleBindings
6. **Least privilege**: Even with ClusterRoles, scope access with RoleBindings when possible
...
2. **Backup operator**: Read secrets and configmaps cluster-wide
3. **Network admin**: Manage NetworkPolicies and Services cluster-wide
4. **Security auditor**: Read all resources including secrets but cannot modify
