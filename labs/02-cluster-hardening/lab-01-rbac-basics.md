...
1. RoleBinding exists: `kubectl get rolebinding <binding> -n <namespace>`
1. RoleBinding references correct role: `kubectl describe rolebinding <binding> -n <namespace>`
1. ServiceAccount name spelled correctly in RoleBinding
1. Namespace matches between Role and RoleBinding
1. API group correct for resource (use `kubectl api-resources`)
...
1. **API groups matter**: Use `kubectl api-resources` to find correct groups
1. **Test everything**: Always use `kubectl auth can-i` to verify
1. **ServiceAccount format**: `system:serviceaccount:NAMESPACE:NAME`
1. **RoleBinding location**: Must be in the same namespace as the Role (for namespace-scoped)
1. **resourceNames limitation**: Doesn't work with list/watch verbs
...
1. **Secret Manager**: Can create and update secrets but not read them
1. **Namespace Admin**: Full access to all resources in namespace except RBAC
1. **Read-Only User**: Can view all resources but not modify anything
