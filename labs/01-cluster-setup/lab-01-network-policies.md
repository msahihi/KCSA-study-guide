...
2. **Why is DNS access required in egress policies?**
   <details>
   <summary>Click to see answer</summary>
...
3. **How would you allow frontend to access an external HTTPS API at api.example.com?**
   <details>
   <summary>Click to see answer</summary>
...
4. **What's the difference between these two policies?**
   ```yaml
   # Policy A
   - from:
     - namespaceSelector:
...
5. **How can you temporarily disable a NetworkPolicy without deleting it?**
   <details>
   <summary>Click to see answer</summary>
...
3. Check NetworkPolicy includes namespaceSelector:
   ```bash
...
2. NetworkPolicies are additive - multiple policies combine
3. Empty podSelector ({}) applies to all pods in namespace
4. Must allow DNS in egress policies
5. Use labels for flexible, maintainable policies
6. Start with default deny, then allow specific traffic
7. Test policies thoroughly before production deployment
8. NetworkPolicies are namespace-scoped resources
9. Both ingress and egress should be controlled
10. Regular audits ensure policies remain effective

## Next Steps

1. Review [Network Policies concept documentation](../../../domains/01-cluster-setup/network-policies.md)
2. Practice creating policies from scratch
3. Proceed to [Lab 02: CIS Benchmarks](./lab-02-cis-benchmarks.md)
