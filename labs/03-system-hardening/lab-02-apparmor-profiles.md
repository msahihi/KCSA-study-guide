...
2. **Profiles are path-based** - easier to understand than SELinux
3. **Three modes**: enforce (block), complain (log), disabled
4. **Use complain mode for development** - discover required permissions
5. **Test thoroughly** - overly restrictive profiles break applications
6. **Deploy to all nodes** - profiles must exist on nodes where pods run
7. **Combine with other security** - AppArmor + Seccomp + capabilities = defense-in-depth
...
2. **Automate profile deployment**: Use a DaemonSet to deploy profiles to all nodes
3. **Generate profiles automatically**: Use `aa-genprof` to create profiles based on observed behavior
4. **Profile a multi-container pod**: Create different profiles for each container in a pod
