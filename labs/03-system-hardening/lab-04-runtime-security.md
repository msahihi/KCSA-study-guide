...
2. **Namespace isolation** - Default isolation is good, but hostNetwork/hostPID are dangerous
3. **Cgroups enforce limits** - Resource limits prevent DoS attacks
4. **Capabilities are powerful** - Drop all capabilities by default, add only what's needed
5. **Privileged containers are dangerous** - Avoid unless absolutely necessary
6. **Read-only root filesystem** - Prevents tampering and persistence
7. **Defense-in-depth** - Combine all security measures for best protection
8. **Monitor runtime events** - Detect suspicious activity early
...
2. **Runtime monitoring**: Set up Falco to monitor your cluster for runtime threats
3. **Custom runtime class**: Create a RuntimeClass for high-security workloads with gVisor
4. **Audit all pods**: Scan your cluster and report which pods use dangerous settings (privileged, hostNetwork, etc.)
